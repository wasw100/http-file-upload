...
import os.path, sys, threading, time, functools, zlib
import mimetypes
import codecs
try:
...
class FileUploadCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        if not local_prefix:
            sublime.status_message('Setting中没有找到local_prefix, 在FileUpload->Settings设置')
        else:
            ...
            if config_key is not None:
                ...
                t = UploadThread(upload_url, file_name, local_prefix, remote_prefix)
                t.start()
            else:
                sublime.status_message(u'没有匹配的配置文件, 可以ctrl+`在Console复制路径 然后在FileUpload->Settings-User设置')


def show_finish(url, remote_path, use_time):
    sublime.status_message(u'上传成功, 服务器-%s, 路径-%s, 使用-%dms' % (url, remote_path, use_time))
...
class UploadThread(threading.Thread):
    ...
    def __init__(self, upload_url, file_name, local_prefix, remote_prefix):
        ...
        local_prefix_length = len(local_prefix)
        if local_prefix.endswith('/'):
            local_prefix_lenth -= 1
        remote_path = os.path.join(remote_prefix, file_name[local_prefix_len+1:])

        self.remote_path = remote_path.replace('\\', '/')
        super(UploadThread, self).__init__()
    
    def run(self):
        data = dict(path=self.remote_path)
        ...
        try:
            ...
            # r = requests.post(self.upload_url, files=files, data=data, timeout=30)
            use_time = (time.time() - begin) * 1000
            callback = functools.partial(show_finish, self.upload_url, self.remote_path, use_time)
            sublime.set_timeout(callback, 0)
        except Exception as e:
            sublime.error_message(u'上传失败, 确认服务器-%s 可以访问,\n 错误信息:%s' % (self.upload_url, e))
            raise e
